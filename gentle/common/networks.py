...
class GaussianMLP(MLP):
    ...
    def process_config(self):
        ...
        self.config.setdefault('apply_tanh', False)
...
class GaussianMLPOffset(GaussianMLP):
    """  Gaussian MLP policy; used for continuous action spaces  """
    def __init__(self, config):
        super().__init__(config)
        self.log_std_offset = Scalar(self.config['log_std_offset'])

    def process_config(self):
        """  Process input configuration  """
        super().process_config()
        self.config.setdefault('log_std_offset', 0.)
    ...
    def forward(self, x):
        ...
        if self.config['log_std_net']:
            if self.config['min_log_std'] is None:
                std = self.log_std_layer(self.mlp(x) + self.log_std_offset()).exp()
            else:
                std = self.log_std_layer(self.mlp(x)+self.log_std_offset()).clamp(min=self.config['min_log_std'],
                                                                                  max=self.config['max_log_std']).exp()
        else:
            if self.config['min_log_std'] is None:
                std = (self.log_std + self.log_std_offset()).exp()
            else:
                std = (self.log_std + self.log_std_offset()).clamp(min=self.config['min_log_std'],
                                                                   max=self.config['max_log_std']).exp()
        return mu, std
