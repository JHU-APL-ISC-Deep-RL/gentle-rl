...
class SoftActorCritic(object):
    ...
    def process_config(self):
        ...
        self.config.setdefault('testing', False)
        ...
        while steps < self.config.total_steps:
            ...
            if steps >= self.config.start_learning and steps % self.config.update_every == 0:
                for i in range(self.config.updates_per_stop):
                    data, latest_loss_info = self.update_networks()
    ...
    def update_networks(self):
        ...
        q_loss.backward()
        q_info['q1_gnorm'] = compute_model_grad_norm(self.q1_network)
        q_info['q2_gnorm'] = compute_model_grad_norm(self.q2_network)
        self.q_optimizer.step()
        q_info['q1_unorm'] = compute_update_norm(old_q1_params, self.q1_network)
        q_info['q2_unorm'] = compute_update_norm(old_q2_params, self.q2_network)
        ...
        pi_loss.backward()
        pi_info['pi_gnorm'] = compute_model_grad_norm(self.pi_network)
        self.pi_optimizer.step()
        pi_info['pi_unorm'] = compute_update_norm(old_pi_params, self.pi_network)
        for p in self.q_params:
    ...
    def compute_q_loss(self, data):
        ...
        q_info = {'q1': torch.mean(q1).item(), 'q2': torch.mean(q2).item(), 'q1_pnorm': compute_model_norm(self.q1_network), 'q2_pnorm': compute_model_norm(self.q2_network)}
